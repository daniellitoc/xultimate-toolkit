		...
		try {
			...
			storageClientTemplate.execute(new AbstractStorageClientCallback() {
				...
				public void doInStorageClient(TrackerClient trackerClient, TrackerServer trackerServer, StorageServer storageServer)
						throws Exception {
			  		System.out.println("delete storage return: " + trackerClient.deleteStorage("group1", "192.168.0.192"));
			  		System.out.println("delete storage errno: " + trackerClient.getErrorCode());
			  		
			  		int count;
					StructGroupStat[] groupStats = trackerClient.listGroups(trackerServer);
					if (groupStats == null) {
						System.out.println("");
						System.out.println("ERROR! list groups error, error no: " + trackerClient.getErrorCode());
						System.out.println("");
						return;
					}

...
public class DataInput extends InputStream {
	...
	public DataInput (int bufferSize) {
	...
	/** Creates a new Input for reading from a byte array.
	 * @param buffer An exception is thrown if more bytes than this are read. */
	public DataInput (byte[] buffer, int offset, int count) {
		setBuffer(buffer, offset, count);
	}

	/** Creates a new Input for reading from an InputStream with a buffer size of 256. */
	...
	public DataInput (InputStream inputStream, int bufferSize) {
		this(bufferSize);
		if (inputStream == null) throw new IllegalArgumentException("inputStream cannot be null.");
		this.inputStream = inputStream;
	}
	...
	private int require (int required) throws IOException {
		int remaining = limit - position;
		if (remaining >= required) return remaining;
		if (required > capacity) throw new IOException("Buffer too small: capacity: " + capacity + ", required: " + required);

		// Try to fill the buffer.
		int count = fill(buffer, limit, capacity - limit);
		if (count == -1) throw new IOException("Buffer underflow.");
		remaining += count;
	...
	private int optional (int optional) throws IOException {
		int remaining = limit - position;
	...
	private int readInt_slow (boolean optimizePositive) throws IOException {
	...
	/** Reads the length and string of UTF8 characters, or null. This can read strings written by {@link DataOutput#writeString(String)}
	 * , {@link DataOutput#writeString(CharSequence)}, and {@link DataOutput#writeAscii(String)}.
	 * @return May be null. 
	 * @throws IOException */
	...
	private int readUtf8Length_slow (int b) throws IOException {
		int result = b & 0x3F; // Mask all but first 6 bits.
		if ((b & 0x40) != 0) { // Bit 7 means another byte, bit 8 means UTF8.
			require(1);
			byte[] buffer = this.buffer;
	...
	private void readUtf8 (int charCount) throws IOException {
		byte[] buffer = this.buffer;
	...
	private void readUtf8_slow (int charCount, int charIndex) throws IOException {
		char[] chars = this.chars;
		byte[] buffer = this.buffer;
	...
	private String readAscii_slow () throws IOException {
	...
	public float readFloat (float precision, boolean optimizePositive) throws IOException {
	...
	private long readLong_slow (boolean optimizePositive) throws IOException {
		// The buffer is guaranteed to have at least 1 byte.
		int b = buffer[position++];
		long result = b & 0x7F;
		if ((b & 0x80) != 0) {
	...
	public double readDouble (double precision, boolean optimizePositive) throws IOException {

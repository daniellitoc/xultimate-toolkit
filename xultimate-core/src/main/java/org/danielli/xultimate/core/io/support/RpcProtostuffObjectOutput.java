...
/**
 * Protostuff序列化机制的对象输出流，针对Date/DateTime、String、Collection、Map、Long/Double/Integer/Float/Character/Short/Byte/Boolean/byte[]特殊处理。
 * 
 * @author Daniel Li
 * @since 10 May 2014
 */
public class RpcProtostuffObjectOutput extends AbstractObjectOutput {
...
//	public static final Schema<MutableObject> SCHEMA = RuntimeSchema.getSchema(MutableObject.class);
	...
	public RpcProtostuffObjectOutput (int bufferSize, LinkedBuffer linkedBuffer, Kryo kryo) {
		super(bufferSize);
		this.buffer = linkedBuffer;
		this.kryo = kryo;
	}

	public RpcProtostuffObjectOutput (int bufferSize, int maxBufferSize, LinkedBuffer linkedBuffer, Kryo kryo) {
		super(bufferSize, maxBufferSize);
		this.buffer = linkedBuffer;
		this.kryo = kryo;
	}

	public RpcProtostuffObjectOutput (byte[] buffer, LinkedBuffer linkedBuffer, Kryo kryo) {
		super(buffer);
		this.buffer = linkedBuffer;
		this.kryo = kryo;
	}

	public RpcProtostuffObjectOutput (byte[] buffer, int maxBufferSize, LinkedBuffer linkedBuffer, Kryo kryo) {
	...
	public RpcProtostuffObjectOutput (OutputStream outputStream, int bufferSize, LinkedBuffer linkedBuffer, Kryo kryo) {

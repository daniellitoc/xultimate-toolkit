...
public class BooleanSerializer extends AbstractClassTypeSupportSerializer {

	@Override
	public boolean support(Class<?> classType) {
		return ClassUtils.isAssignable(Boolean.class, classType);
	}
	
	@Override
	public <T> byte[] serialize(T source) throws SerializerException {
		return SerializerUtils.encodeBoolean((Boolean) source);
	}

	@Override
	public <T> void serialize(T source, OutputStream outputStream) throws SerializerException {
		try {
			outputStream.write(SerializerUtils.encodeBoolean((Boolean) source));
		} catch (IOException e) {
	...
	public <T> T deserialize(byte[] bytes, Class<T> clazz) throws DeserializerException {
		return clazz.cast(SerializerUtils.decodeBoolean(bytes));
	}
	...
	public <T> T deserialize(InputStream inputStream, Class<T> clazz) throws DeserializerException {
		try {
			...
			return clazz.cast(SerializerUtils.decodeBoolean(result));
		} catch (IOException e) {

 ...
 * 
 */
public class RpcProtostuffSerializer extends RpcSerializer {
...
//				MutableObject<T> holder = new MutableObject<T>(source);
				...
				try {
//					ProtostuffIOUtil.writeDelimitedTo(output, holder, schema, buffer);
					ProtostuffIOUtil.writeDelimitedTo(output, source, RuntimeSchema.getSchema(clazz), buffer);
				} catch (Exception e) {
...
//				MutableObject<T> holder = new MutableObject<T>();
				LinkedBuffer buffer = LinkedBufferUtils.getCurrentLinkedBuffer(bufferSize);
				try {
//					ProtostuffIOUtil.mergeDelimitedFrom(input, holder, schema, buffer);
					T result = schema.newMessage();
					ProtostuffIOUtil.mergeDelimitedFrom(input, result, schema, buffer);
					return result;
				} catch (Exception e) {
